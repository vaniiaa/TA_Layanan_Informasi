{{-- resources/views/admin/p2m/materi_edukasi.blade.php --}}
                        ...
                        <tr>
                            ...
                            <td class="flex gap-2">
                                ...
                                <form id="delete-form-{{ $item->id }}" method="POST"
                                    action="{{ route('admin.materi.destroy', $item->id) }}"
                                    class="hidden">
                                    @csrf
                                    @method('DELETE')
                                </form>
    ...
    <div class="bg-white w-full max-w-lg rounded-xl shadow-lg p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form action="{{ route('admin.materi.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div id="formModulTambah" class="hidden">
                ...
                <label class="font-semibold">Upload Modul<span class="text-red-500">*</span></label>
                <input type="file" name="file_modul" class="file-input w-full" accept=".pdf,application/pdf">
            ...
            <div id="formVideoTambah" class="hidden">
                ...
                <input type="file" name="video_file" class="file-input w-full" accept=".mp4,video/mp4">
...
<div id="modalDetail" class="fixed inset-0 hidden z-[9999] bg-black/60 flex items-center justify-center">
    ...
    <div class="bg-white w-full max-w-lg rounded-xl shadow-lg p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div class="space-y-4 text-sm">
            ...
            {{-- MEDIA (SELALU DI TEMPAT YANG SAMA) --}}
            <div id="detailMedia">
                {{-- Diisi JS:
...
{{-- ================= MODAL HAPUS ================= --}}
<div id="modalHapus" class="fixed inset-0 hidden z-[9999] bg-black/60 flex items-center justify-center">
    <div class="bg-white rounded-xl p-6 shadow-lg text-center max-w-sm">
        <i class="fa-solid fa-circle-exclamation text-red-600 text-5xl mb-3"></i>
        <h3 class="font-bold text-lg">Hapus Materi?</h3>
        <p class="text-sm mt-2">
            Yakin hapus <b id="hapusJudul"></b>?
        </p>
        <form id="formDelete" method="POST">
            @csrf
            @method('DELETE')
            <div class="flex justify-center gap-3 mt-5">
                <button type="button" onclick="closeModal('modalHapus')" class="btn bg-gray-200">
                    Batal
                </button>
...
function showEdit(item){
    document.getElementById('formEdit').action = `/admin/p2m/materi_edukasi/${item.id}`
